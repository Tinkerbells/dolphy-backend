...
export class MarketDecksService {
  ...
  async create(
    ...
    newMarketDeck.title = createMarketDeckDto.title;
    newMarketDeck.description = createMarketDeckDto.description;
    newMarketDeck.tags = createMarketDeckDto.tags || [];
    newMarketDeck.isPublic = createMarketDeckDto.isPublic ?? true;
    newMarketDeck.isCopyAllowed = createMarketDeckDto.isCopyAllowed ?? true;
  ...
  async update(
    ...
    if (updateMarketDeckDto.title !== undefined) {
      updateData.title = updateMarketDeckDto.title;
    }

    if (updateMarketDeckDto.description !== undefined) {
      updateData.description = updateMarketDeckDto.description;
    }

    if (updateMarketDeckDto.tags !== undefined) {
      updateData.tags = updateMarketDeckDto.tags;
    }

    if (updateMarketDeckDto.isPublic !== undefined) {
      updateData.isPublic = updateMarketDeckDto.isPublic;
    }

    if (updateMarketDeckDto.isCopyAllowed !== undefined) {
      updateData.isCopyAllowed = updateMarketDeckDto.isCopyAllowed;
    }
  ...
  async copyDeck(
    ...
    // Проверяем, разрешено ли копирование
    if (!marketDeck.isCopyAllowed) {
      throw new ForbiddenException('Копирование этой колоды запрещено');
    }
    ...
    const copiedDeck = await this.decksService.create(
      {
        name: `Копия: ${marketDeck.title}`,
        description: `${marketDeck.description}\n\nСкопировано из маркетплейса`,
      },
      userId,
    );
